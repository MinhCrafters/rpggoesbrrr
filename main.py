...
async def info(ctx):
    info = fileIO("config/config.json", "load")
    em = discord.Embed(title="My info:", type="rich",
                       description="1) Prefix: {}\n2) Name: {}\n3) User ID: {}\n4) Version: {}\n5) Shards: {}\n6) "
                                   "Total commands used: {}".format(
                           Prefix, bot.user.name, bot.user.id, VS, Shards, info["Commands_used"]),
                       color=ctx.author.color)
    em.set_imagician(url="https://media.giphy.com/media/3og0IzI7ASX3mW5csg/giphy.gif")
...
async def start(ctx):
    ...
    if info["race"] and info["class"] == "None":
        await ctx.send(
            "<@{}> Stats created.\n\nWelcome to Reimu Hakurei's RPG!\nMay I ask what race you are?\n`Choose one of "
            "the following`\nGhost\nHuman\nYoukai".format(author.id))

        def pred(m):
            return m.author == message.author and m.channel == message.channel

        answer1 = await bot.wait_for("message", check=pred)
        values = ["Ghost", "ghost", "human", "Human", "youkai", "Youkai", "{}start".format(Prefix)]
        if str(answer1.content) in values:
            if answer1.content == "{}start".format(Prefix):
                return
            elif answer1.content == "Ghost" or answer1.content == "ghost":
                info["race"] = "Ghost"
                ...
                fileIO("players/{}/info.json".format(author.id), "save", info)
                await _pick_class(ctx)
            elif answer1.content == "Youkai" or answer1.content == "youkai":
                info["race"] = "Youkai"
                fileIO("players/{}/info.json".format(author.id), "save", info)
                await _pick_class(ctx)
        else:
...
async def fight(ctx):
    ...
    if info["race"] and info["class"] == "None":
        ...
        return
    if info["health"] <= 0:
        await ctx.send("<@{}> You cannot fight as a dead.".format(author.id))
        return
    if info["selected_enemy"] == "None":
        ...
        if elocation == "Saker Keep":
            monster = ["Draugr", "Stalker", "Soul-eater"]
        ...
        await ctx.send(
            "You wonder around {} and find a {}, would you like to fight it?\n**Y** or **N**".format(info["location"],
                                                                                                     enemy))
        ...
        if str(answer1.content) in options:
            if answer1.content == "Stab" or answer1.content == "stab":
                ...
                move = "Shoot"
            # lootbox# 10% chance to obtain one from an enemy.
            lootbox = random.randint(1, 10)
            ...
            await ctx.send(
                "```diff\n- {} has {} HP\n+ {} has {} HP\n\n- {} hits {} for {} dmg.\n+ {} uses {} and hits for {} "
                "dmg.\n\n- {} has {} hp left\n+ {} has {} hp left.```".format(
                    user_enemy, user_enemy_hp, user_name, user_hp, user_enemy, user_name, enemy_dmg, user_name, move,
                    user_dmg, user_enemy, enemy_hp_after, user_name, user_hp_after))
            ...
            if enemy_hp <= 0 and user_hp <= 0:
                ...
                if lootbox == 6:
                    await ctx.send("```diff\n+ {} obtained a lootbox!```".format(user_name))
                    info["lootbox"] = info["lootbox"] + 1
                    fileIO("players/{}/info.json".format(author.id), "save", info)
...
async def inv(ctx):
    ...
    em = discord.Embed(
        description="```diff\n!======== [{}'s Inventory] ========!\n\n!==== [Supplies] ====!\n+ Gold : {}\n+ Wood : {}\n+ Stone : {}\n+ Metal : {}\n\n!===== [Items] =====!\n+ Keys : {}\n+ Loot Bags : {}\n+ Minor HP Potions : {}\n+ {}```".format(
            info["name"], info["gold"], info["wood"], info["stone"], info["metal"], info["keys"], info["lootbox"],
            info["hp_potions"], "\n+ ".join(info["inventory"])), color=ctx.author.color)
    await ctx.send(embed=em)
...
async def stats(ctx):
    ...
    em = discord.Embed(
        description="```diff\n!======== [{}'s Stats] ========!\n+ Name : {}\n+ Title : {}\n+ Race : {}\n+ Class : {}\n\n+ Level : {} | Exp : ({}/{})\n+ Health : ({}/100)\n+ Stamina : {}\n+ Mana : {}\n\n!===== [Equipment] =====!\n+ Weapon : {}\n+ Wearing : {}\n\n+ Killed : {} Enemies\n+ Died : {} Times```".format(
            info["name"], info["name"], info["title"], info["race"], info["class"], info["lvl"], info["exp"], maxexp,
            info["health"], info["stamina"], info["mana"], info["equip"], info["wearing"], info["enemieskilled"],
            info["deaths"]), color=ctx.author.color)
    await ctx.send(embed=em)
    ...
    if len(inv_list) == 0:
        ...
        em = discord.Embed(
            description="```diff\n+ What would you like to equip?\n- Note: this is Uppercase and Lowercase sensitive.\n{}```".format(
                "\n".join(inv_list)), color=ctx.author.color)
        await ctx.send(embed=em)
        ...
        if answer1.content in inv_list:
            em = discord.Embed(description="```diff\n+ You equip the {}!```".format(answer1.content),
                               color=ctx.author.color)
            await ctx.send(embed=em)
...
async def lootbox(ctx):
    ...
    if info["race"] and info["class"] == "None":
        ...
        return
    if info["lootbox"] == 0:
        em = discord.Embed(description="```diff\n- You don't have any lootboxes!```", color=ctx.author.color)
        await ctx.send(embed=em)
        return
    else:
        em = discord.Embed(description="```diff\n+ {} starts opening a lootbox. . .```".format(info["name"]),
                           color=ctx.author.color)
        await ctx.send(embed=em)
        ...
        if chance == 3:
            em = discord.Embed(description="```diff\n+ They obtained {} Gold from the lootbox!```".format(goldgain),
                               color=ctx.author.color)
            await ctx.send(embed=em)
            ...
            em = discord.Embed(description="```diff\n- The lootbox didn't contain anything!```",
                               color=ctx.author.color)
            await ctx.send(embed=em)
            info["lootbox"] = info["lootbox"] - 1
            fileIO("players/{}/info.json".format(author.id), "save", info)
...
async def travel(ctx):
    ...
    em = discord.Embed(
        description="<@{}>\n```diff\n+ Where would you like to travel?\n- Type a location number in the chat.\n+ {}```".format(
            author.id, "\n+ ".join(options)), color=ctx.author.color)
    await ctx.send(embed=em)
    ...
    if answer1.content in options2:
        if answer1.content == "0":
            if info["location"] == "Golden Temple":
                em = discord.Embed(
                    description="<@{}>\n```diff\n- You're already at {}!```".format(author.id, info["location"]),
                    color=discord.Color.red())
                await ctx.send(embed=em)
                return
            else:
        ...
        em = discord.Embed(description="<@{}>\n```diff\n+ Traveling to {}...```".format(author.id, location_name),
                           color=discord.Color.red())
        await ctx.send(embed=em)
        await asyncio.sleep(3)
        info["location"] = location_name
        fileIO("players/{}/info.json".format(author.id), "save", info)
        em = discord.Embed(description="<@{}>\n```diff\n+ You have arrived at {}```".format(author.id, location_name),
                           color=discord.Color.red())
        await ctx.send(embed=em)
    else:
...
async def buy(ctx):
    ...
    if answer1.content in options:
        if answer1.content == "{}buy".format(Prefix):
            ...
            em = discord.Embed(description="```diff\n+ How many would you like to buy?```", color=ctx.author.color)
            await ctx.send(embed=em)
            ...
            try:
                ...
                if info["gold"] < value:
                    em = discord.Embed(
                        description="```diff\n- {}, you don't have enough gold for {} potion(s)```".format(info["name"],
                                                                                                           answer2.content),
                        color=discord.Color.red())
                    await ctx.send(embed=em)
                else:
                    ...
                    fileIO("players/{}/info.json".format(author.id), "save", info)
                    em = discord.Embed(
                        description="```diff\n+ {}, you bought {} potion(s) for {} gold!```".format(info["name"],
                                                                                                    answer2.content,
                                                                                                    value),
                        color=ctx.author.color)
                    await ctx.send(embed=em)
                ...
                em = discord.Embed(
                    description="```diff\n- {}, please put a correct number value next time.```".format(info["name"]),
                    color=discord.Color.red())
                await ctx.send(embed=em)
        elif answer1.content == "Items" or answer1.content == "items":
            if info["class"] == "magician":
                items = ["sprine staff", "Sprine staff", "{}buy".format(Prefix)]
                em = discord.Embed(
                    description="**Items for {} class:**\nItem - Cost\n```diff\n- Sprine Staff | 1,000 gold.```\n```diff\n+ What would you like to buy?```".format(
                        info["class"], color=ctx.author.color))
                await ctx.send(embed=em)
                ...
                if answer3.content == "{}buy".format(Prefix):
                    ...
                    if info["gold"] < 1000:
                        em = discord.Embed(
                            description="```diff\n- {}, you don't have enough gold for the Sprine Staff.```".format(
                                info["name"]), color=discord.Color.red())
                        await ctx.send(embed=em)
                    else:
                        ...
                        fileIO("players/{}/info.json".format(author.id), "save", info)
                        em = discord.Embed(
                            description="```diff\n+ {}, you bought the Sprine Staff!```".format(info["name"]),
                            color=ctx.author.color)
                        await ctx.send(embed=em)
                else:
                    em = discord.Embed(
                        description="```diff\n- {}, please state a correct item next time.```".format(info["name"]),
                        color=discord.Color.red())
                    await ctx.send(embed=em)
            elif info["class"] == "shrine maiden":
                ...
                if answer3.content == "{}buy".format(Prefix):
                    ...
                    if info["gold"] < 1000:
                        em = discord.Embed(
                            description="```diff\n- {}, you don't have enough gold for the Sprine Sword.```".format(
                                info["name"]), color=discord.Color.red())
                        await ctx.send(embed=em)
                    else:
                        info["gold"] = info["gold"] - 1000
                        ...
                        fileIO("players/{}/info.json".format(author.id), "save", info)
                        em = discord.Embed(
                            description="```diff\n+ {}, you bought the Sprine Sword!```".format(info["name"]),
                            color=ctx.author.color)
                        await ctx.send(embed=em)
                else:
                    em = discord.Embed(
                ...
                if answer3.content == "{}buy".format(Prefix):
                    ...
                    if info["gold"] < 1000:
                        em = discord.Embed(
                            description="```diff\n- {}, you don't have enough gold for the Sprine Dagger.```".format(
                                info["name"]), color=discord.Color.red())
                        await ctx.send(embed=em)
                    else:
                        info["gold"] = info["gold"] - 1000
                        ...
                        fileIO("players/{}/info.json".format(author.id), "save", info)
                        em = discord.Embed(
                            description="```diff\n+ {}, you bought the Sprine Dagger!```".format(info["name"]),
                            color=ctx.author.color)
                        await ctx.send(embed=em)
                else:
                    em = discord.Embed(
                ...
                if answer3.content == "{}buy".format(Prefix):
                    ...
                    if info["gold"] < 1000:
                        em = discord.Embed(
                            description="```diff\n- {}, you don't have enough gold for the Sprine Bow.```".format(
                                info["name"]), color=discord.Color.red())
                        await ctx.send(embed=em)
                    else:
                        info["gold"] = info["gold"] - 1000
                        ...
                        fileIO("players/{}/info.json".format(author.id), "save", info)
                        em = discord.Embed(
                            description="```diff\n+ {}, you bought the Sprine Bow!```".format(info["name"]),
                            color=ctx.author.color)
                        await ctx.send(embed=em)
                else:
                    em = discord.Embed(
            ...
            em = discord.Embed(
                description="```diff\n- {}, please put a correct value next time.```".format(info["name"]),
                color=discord.Color.red())
            await ctx.send(embed=em)
    else:
        em = discord.Embed(description="```diff\n- {}, please put a correct value next time.```".format(info["name"]),
...
async def rest(ctx):
    ...
    if delta >= 120.0 and delta > 0:
        ...
        fileIO("players/{}/info.json".format(author.id), "save", info)
        em = discord.Embed(description="```diff\n+ You gained {} HP for resting!```".format(HPget),
                           color=ctx.author.color)
        await ctx.send(embed=em)
    else:
        seconds = 120 - delta
...
async def mine(ctx):
    ...
    if delta >= 600.0 and delta > 0:
        ...
        fileIO("players/{}/info.json".format(author.id), "save", info)
        em = discord.Embed(
            description="```diff\n+ You mined a Rock!\n+ {} Metal\n+ {} Stone```".format(mined_metal, mined_rock),
            color=ctx.author.color)
        await ctx.send(embed=em)
    else:
        seconds = 600 - delta
...
async def _pick_class(ctx):
    ...
    values2 = ["archer", "Archer", "shrine maiden", "Shrine Maiden", "magician", "Magician", "thief", "Thief", "{}start".format(Prefix)]
    if str(answer2.content) in values2:
        if answer2 == "{}start".format(Prefix):
            ...
            return
        elif answer2.content == "shrine maiden" or answer2.content == "Shrine Maiden":
            info["class"] = "Shrine Maiden"
            info["skills_learned"].append("Swing")
            info["equip"] = "Simple Sword"
            fileIO("players/{}/info.json".format(author.id), "save", info)
            await ctx.send("All setup!")
            return
        elif answer2.content == "magician" or answer2.content == "Magician":
            info["class"] = "Magician"
